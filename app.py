...
# Kolom yang digunakan dalam scaling
...
if prediksi_button:
    ...
    for col in ['AlcoholConsumption', 'SnackConsumption']:
        data_input[col] = data_input[col].map(ordinal_maps[col])

    # Encoding dengan LabelEncoder
    for col in ['Gender', 'HighCalorieFood', 'CalorieMonitoring', 'FamilyHistoryOverweight', 'Transportation']:
        data_input[col] = encoders[col].transform(data_input[col])

    # Scaling numerik
    scaled_data = scaler.transform(data_input[scaled_columns])

    # Gabungkan fitur akhir
    fitur_akhir = np.hstack([
        scaled_data,
        data_input[['AlcoholConsumption', 'SnackConsumption']].values
    ])
